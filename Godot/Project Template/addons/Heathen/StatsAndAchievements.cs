        ...
        public static class Client
        {
            public static void Initialize()
            {
                if (m_UserStatsReceived_t == null)
                    m_UserStatsReceived_t = Callback<UserStatsReceived_t>.Create(StatsReceived);

                if (m_UserStatsUnload_t == null)
                    m_UserStatsUnload_t = Callback<UserStatsUnloaded_t>.Create(StatsUnloaded);

                if (m_UserStatsStored_t == null)
                    m_UserStatsStored_t = Callback<UserStatsStored_t>.Create(StatsStored);

                if (m_UserAchievementStored_t == null)
                    m_UserAchievementStored_t = Callback<UserAchievementStored_t>.Create(AchievementStored);
                ...
                loadedImages = new Dictionary<int, Image>();
            ...
            private static void AchievementStored(UserAchievementStored_t param)
            {
                EventUserAchievementStored?.Invoke(param);
            }

            private static void StatsStored(UserStatsStored_t param)
            {
                EventUserStatsStored?.Invoke(param);
            }

            private static void StatsUnloaded(UserStatsUnloaded_t param)
            {
                EventUserStatsUnloaded?.Invoke(param);
            }

            private static void StatsReceived(UserStatsReceived_t param)
            {
                EventUserStatsReceived?.Invoke(param);
            }
            ...
            private static Dictionary<int, Image> loadedImages = new Dictionary<int, Image>();
            public static event UserStatsReceivedEvent EventUserStatsReceived;
            public static event UserStatsUnloadedEvent EventUserStatsUnloaded;
            public static event UserStatsStoredEvent EventUserStatsStored;
            public static event UserAchievementStoredEvent EventUserAchievementStored;
            ...
            public static void GetAchievementIcon(string achievementApiName, Action<Image> callback)
            {
                if (callback == null)
                    return;
                ...
                if (handle > 0)
                {
                    ...
                    else
                    {
                        ...
                        {
                            GD.Print("Failed to load the requested avatar");
                ...
                {
                    GD.Print("No avatar available for this user");
            ...
            public static void GetMostAchievedAchievements(Action<EResult, (Achievement achievement, float percentage)[], bool> callback)
            {
                RequestGlobalAchievementPercentages((result, error) =>
                {
                    if (!error && result.m_eResult == EResult.k_EResultOK)
                    {
                        ...
                        if (index > -1)
                        {
                            List<(Achievement achievement, float percentage)> results = new List<(Achievement achievement, float percentage)>();
            ...
            private static bool LoadImage(int imageHandle)
            {
                if (SteamUtils.GetImageSize(imageHandle, out uint width, out uint height))
                {
                    ...
                    {
                        pointer.CreateFromData((int)width, (int)height, false, Image.Format.Rgba8, imageBuffer);
                    }
