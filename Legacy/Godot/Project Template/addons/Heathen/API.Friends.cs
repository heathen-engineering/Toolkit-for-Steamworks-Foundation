        ...
        public static class Client
        {
            public static void  Initialize()
            {
                if (m_GameConnectedFriendChatMsg_t == null)
                    m_GameConnectedFriendChatMsg_t = Callback<GameConnectedFriendChatMsg_t>.Create(FriendMessageRecieved);

                if (m_FriendRichPresenceUpdate_t == null)
                    m_FriendRichPresenceUpdate_t = Callback<FriendRichPresenceUpdate_t>.Create(RichPresenceUpdated);

                if (m_PersonaStateChange_t == null)
                    m_PersonaStateChange_t = Callback<PersonaStateChange_t>.Create(HandlePersonaStateChange);
            }

            private static void RichPresenceUpdated(FriendRichPresenceUpdate_t param)
            {
                EventFriendRichPresenceUpdate?.Invoke(param);
            }

            private static void FriendMessageRecieved(GameConnectedFriendChatMsg_t param)
            {
                var count = SteamFriends.GetFriendMessage(param.m_steamIDUser, param.m_iMessageID, out string message, 8193, out EChatEntryType type);
                if (count > 0)
                    EventFriendMessageReceived?.Invoke(param.m_steamIDUser, message, type);
            }
            ...
            public static event GameConnectedFriendChatMsgEvent EventFriendMessageReceived;
            private static bool listeningForFriendMessages = false;
            public static event PersonaStateChangeEvent EventPersonaStateChange;
            public static event FriendRichPresenceUpdateEvent EventFriendRichPresenceUpdate;
            ...
            private static Dictionary<CSteamID, Image> userAvatarMapping = new Dictionary<CSteamID, Image>();
            ...
            public static void GetFriendAvatar(UserData userId, Action<Image> callback)
            {
                if (callback == null)
                    return;
            ...
            public static void UnloadAvatarImage(Image image)
            {
            ...
            public static Image GetLoadedAvatar(UserData id)
            {
            ...
            private static void HandleAvatarImageLoaded(AvatarImageLoaded_t results)
            {
                ...
                {
                    GD.Print("Steam API responded with an Avatar Loaded [" + results.m_iImage + "] message for user [" + results.m_steamID.m_SteamID + "] however no avatar was found on the local disk.");
                }
            }
